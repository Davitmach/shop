...
window.addEventListener('resize',()=> {
  if(window.innerWidth <=552) {
    function setPosition1() {
      ...
      thumb1.style.transform = '  translateX(' + position + 'px)';
      displayValues();
      setRangeBar();
    }
    function setPosition2() {
      let position = (value2 - minValue) / valueRange * sliderContainerWidth;
      thumb2.style.transform = '  translateX(' + position + 'px)';
      displayValues();
      setRangeBar();
    }
  }
...
function displayValues() {
  ...
  displayValue1.value = Math.round(minRange);
  displayValue2.value = Math.round(maxRange);
}
